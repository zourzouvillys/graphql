...
class ZuluExecutableUtils {
  ...
  private static final MethodHandle MH_close = ZuluUtils.unreflect(
      MethodHandles.lookup(),
      ResultWriter.class,
      "close");

  private static final MethodHandle MH_ctor = ZuluUtils
      .sneakyThrows(() -> lookup().findConstructor(
          ResultWriter.class,
  ...
  private static final MethodHandle MH_write = ZuluUtils.unreflect(
      MethodHandles.lookup(),
      ResultWriter.class,
      "write",
      ZuluSelection.class);

  private static final MethodHandle MH_write_object = ZuluUtils.unreflect(
      MethodHandles.lookup(),
      ResultWriter.class,
      "write",
      ZuluSelection.class,
  ...
  private static final MethodHandle MH_write_boolean = ZuluUtils.unreflect(
      MethodHandles.lookup(),
      ResultWriter.class,
      "write",
      ZuluSelection.class,
  ...
  private static final MethodHandle MH_write_int = ZuluUtils.unreflect(
      MethodHandles.lookup(),
      ResultWriter.class,
      "write",
      ZuluSelection.class,
  ...
  private static final MethodHandle MH_write_long = ZuluUtils.unreflect(
      MethodHandles.lookup(),
      ResultWriter.class,
      "write",
      ZuluSelection.class,
  ...
  private static final MethodHandle MH_write_String = ZuluUtils.unreflect(
      MethodHandles.lookup(),
      ResultWriter.class,
      "write",
      ZuluSelection.class,
  ...
  private static MethodHandle reduce(
      ExecutableBuilder exb,
      ExecutableOutputField field,
      ZuluSelection sel,
      MethodHandle returnValue,
      Map<ZuluSelection, MethodHandle> childFields) {
    ...
    // the first value is the last call: get return value from result writer, and close it.
  ...
  /**
   * generates a {@link MethodHandle} to execute and write the result of this field to the stage handler.
   * 
   * 
   * 
   * @param subsel
   * @param childHandler
   *          The handle to invoke to generate the child field value. signature (ZuluResultReceiver, C)T.
   * @param returnValue
   * @return a method handle with signature (ResultWriter, ZuluResultsReceiver, C)void
   */

  private static MethodHandle createWriteFolder(GQLPreparedSelection subsel, MethodHandle childHandler, Class<?> stageContextType) {

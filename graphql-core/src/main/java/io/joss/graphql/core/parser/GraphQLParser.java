...
/**
 * Although a parser generator could be used, the amount of code is pretty much identical due to the well designed nature of the GraphQL
 * specification.
 *
 * @author theo
 *
...
public class GraphQLParser
{
  ...
  /**
   * Parses the given string into a document model and performs basic structural validation, but doesn't validate the shape or other query
   * semantics.
   *
   * @param doc
   * @return
  ...
  private GQLDocument validate(GQLDocument doc)
  {
    ...
    return doc.withDefinitions(doc.definitions().stream().map(def -> def.apply(visitor)).collect(Collectors.toList()));
  }

  /**
   * Parses a query, which must begin with 'query' or '{'.
   * 
   * Note that this is only useful for diagnostics and debugging, as the returned query is not validated and any references to fragments
  ...
  {
    return new GQLSchemaBuilder().add(GQLTypes.builtins()).add(new ParseContext(schema).parseSchema()).build();
  }
  ...
  {
    return new GQLSchemaBuilder().add(GQLTypes.builtins()).add(new ParseContext(streamToString(schema)).parseSchema()).build();
  }

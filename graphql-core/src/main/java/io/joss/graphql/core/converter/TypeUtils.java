...
public class TypeUtils
{
  ...
  /**
   * Given an input class, finds the usage of 'wanted', and returns the annotated type number of param.
   */
  ...
  public static AnnotatedType getParamterOfInterface(Class<?> klass, Class<?> wanted, int param)
  {
    ...
    AnnotatedParameterizedType ptype = (AnnotatedParameterizedType) val;
  ...
  /**
   * Finds the {@link AnnotatedType} usage for a class which implements the given interface.
   * 
   * e.g, MyService<XXX> will return an annotatedType for MyService<XXX> if you call getInterfaceType(myobj, MyService.class);
   * 
   * @param klass
   * @param iface
   * 
  ...
  public static AnnotatedType getInterfaceType(Class<?> klass, Class<?> wanted)
  {
    ...
    for (AnnotatedType iface : klass.getAnnotatedInterfaces())
    {
